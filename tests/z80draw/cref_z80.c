/* cref_z80.c - zsdcc-compiled C copy of the plat_next.c DDA, exporting
 * the same plat_draw_line_asm symbol as draw_z80n.asm.
 *
 * Two purposes:
 *   1. cycle baseline: run.sh links the wrapper against this instead of
 *      the asm and compares z88dk-ticks cycle counts (the ">= 2x on the
 *      inner loop" bar);
 ...
 *      the asm (and doubling as the documented ticks fallback strategy).
 *
 * The body is a verbatim copy of the pre-asm plat_next.c rasterizer
 * (which itself mirrors core/draw_ref.c), with row_addr built lazily
 * since there is no plat_init in the test image.
 */
#include <stdint.h>
