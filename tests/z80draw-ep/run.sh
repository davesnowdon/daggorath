#!/usr/bin/env bash
# run.sh - build and run the EP z80 draw identity test.
#
#   1. generate the deterministic corpus (corpus_gen_ep.c: the Next's
#      corpus categories 1-7 byte-identical, plus EP-directed category 8
#      hammering the H/V solid fast paths)
#   2. compile the REAL enterprise/draw_ep.asm into a zsdcc test
#      wrapper (z88dk +test target, org 0x8000) and execute it under
#      z88dk-ticks -mz80 (PLAIN Z80 - proves no Z80n opcodes survive);
#      collect the 64K RAM dump on exit
#   3. rasterize the same corpus with the normative core/draw_ref.c on
#      the host (gcc), pack to the EP linear layout, compare
#      byte-for-byte
#   4. repeat step 2/3 with a zsdcc-compiled C DDA (cref_ep.c) for a
#      cycle-count baseline and an independent harness cross-check
#
# Prints "Z80DRAW-EP IDENTICAL" on success (asm side), else a diff
# summary.
set -euo pipefail
cd "$(dirname "$0")"
...
CORE=../../core
ASM=../../enterprise/draw_ep.asm

echo "== 1. corpus"
gcc -std=c99 -O2 -Wall -Wextra -o corpus_gen_ep corpus_gen_ep.c
./corpus_gen_ep > corpus_data.h

echo "== 2. host reference (gcc + core/draw_ref.c)"
gcc -std=c99 -O2 -Wall -Wextra -I"$CORE" -o host_check \
    host_check.c "$CORE/draw_ref.c"

# Org 0x5C00, NOT the Next harness's 0x8000: this corpus is ~6KB larger
# (category 8) and at 0x8000 the image tail crossed 0xFFFF - the wrapped
# BSS then silently corrupted the corpus tail (one-pixel heisen-diffs).
# 0x5C00 sits just above the sentinel (0x5B00); the image ends ~0xE6xx.
ORG=0x5C00
...
build_and_run () {
    ...
    # ticks v0.14c quirks (all verified empirically, see ../z80draw):
    #   - segfaults on input files without an extension -> use .bin
    #   - default run cap is 1e8 cycles; raise with -counter, NOT -w
    #   - the RAM dump is only written on -end/-counter exits
    cp "wrapper_$tag" "wrapper_$tag.bin"
    rm -f "ram_$tag.bin"
    timeout 300 z88dk-ticks -mz80 -l $ORG -pc $ORG -end "0x$done_addr" \
...
echo "== 3. EP asm under z88dk-ticks (-mz80: plain Z80)"
