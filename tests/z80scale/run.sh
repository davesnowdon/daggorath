#!/usr/bin/env bash
# run.sh - build and run the z80 volume-scale identity test.
#
#   1. compile the REAL spectrum-next/snd_scale.asm into a zsdcc test
#      wrapper (z88dk +test target, org 0x8000) and execute it under
#      z88dk-ticks -mz80n; collect the 64K RAM dump on exit
#   2. replicate the wrapper's LFSR corpus + LUT scaling with gcc on
#      the host and compare the dump byte-for-byte
#
# Prints "Z80SCALE IDENTICAL" on success, else a diff summary.
...
ASM=../../spectrum-next/snd_scale.asm

echo "== 1. z80 asm under z88dk-ticks"
zcc +test -clib=z80n -compiler=sdcc -SO2 --max-allocs-per-node20000 \
    -pragma-define:CRT_ORG_CODE=0x8000 \
    wrapper.c done.asm "$ASM" -o wrapper_asm -m
...
echo "== 2. verdict (asm vs gcc reference)"
gcc -std=c99 -O2 -Wall -Wextra -o host_check host_check.c
